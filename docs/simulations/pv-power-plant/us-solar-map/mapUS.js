...
export async function initHeatmap(width) {
    ...
    csvData.raw = await getCSVData("/simulations/pv-power-plant/data/uspvdb_v4_0_20260414_lite.csv");
...
async function getCSVData(csvPath) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        const area = cols[2];
        const mW = cols[3];
        const [x, y] = window.proj4("EPSG:4326", lambertUSA, [lon, lat]);
        data[i - 1] = { x: x, y: y, mW: mW, area: area }; 
    }
    return data;
}

function resizeData(scale, xOffset, yOffset, radius) {
    ...
    for (let i = 0; i < rawLength; i++) {
        const r = raw[i];
        const scaledX = r.x * scale + xOffset;
        const scaledY = -r.y * scale + yOffset;
...
export function updateHeatmap(scale, xOffset, yOffset, radius, max = 0, xShift = 0, yShift = 0) {
    const dataLen = resizeData(csvData.width / 1000 * csvData.scale * scale, csvData.width * xOffset + xShift, csvData.width * yOffset + yShift, radius);
    console.log(dataLen);
